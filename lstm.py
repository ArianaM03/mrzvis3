...
if __name__ == '__main__':
    ...
    LSTM = tf.keras.models.Sequential([
        tf.keras.layers.InputLayer(input_shape=(succession_len, 1)),
        tf.keras.layers.LSTM(steps_num, activation=func_of_activation, return_sequences=False),
        tf.keras.layers.Dense(units=1)
    ])

    LSTM.compile(loss=tf.losses.Huber(),
    ...
    train_lstm(LSTM, list_for_training, steps_num)
    forecast = test_lstm(LSTM, testing_set)
